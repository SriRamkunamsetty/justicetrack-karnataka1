# Justice Track Karnataka
## AMD GPU-Accelerated Legal NLP and Case Summarization
...
GitHub Repository:
https://github.com/SriRamkunamsetty/justicetrack-karnataka1/tree/main/Justice Track Karnataka AMD submission

*Confidentiality boundary: This paper describes Justice Track Karnataka at a high infrastructure level only. Confidential architecture components, proprietary orchestration logic, internal optimization modules, agent decision workflows, and patent-sensitive implementation specifics are omitted.*

---

## Abstract

Justice Track Karnataka is a modular AI-powered framework designed for dense legal document parsing, precedent clustering, and LLM-based case summarization, accelerated through AMD GPU infrastructure. This white paper presents the system as an enterprise-grade AI infrastructure case study centered on AMD Instinct GPUs, the ROCm software ecosystem, and GPU-accelerated inference engineering. The emphasis is not on proprietary algorithmic internals, but on reproducible deployment practices, AMD GPU utilization, scalable inference design, latency-aware pipeline construction, and review-ready benchmarking methodology.
...
This paper defines a safe public architecture for Justice Track Karnataka, outlines ROCm integration, proposes benchmark methodology, includes enterprise-style performance tables, and specifies a reproducible GitHub repository structure suitable for AMD AI Engage technical review.
...
```text
Application Layer
 Justice Track Karnataka public inference interfaces
...
MI300X is a discrete data center GPU accelerator designed for demanding AI workloads. Its most important characteristic is memory density: up to 192 GB HBM3 per accelerator, allowing Justice Track Karnataka to keep large models and buffers resident in GPU memory.

### 5.2 AMD Instinct MI300A
MI300A integrates 24 Zen 4 CPU cores with 228 CDNA 3 GPU compute units and 128 GB HBM3. It is relevant for tightly coupled CPU-GPU analytics pipelines within Justice Track Karnataka.

## 6. Justice Track Karnataka Overview
...
```text
PDF Intake -> OCR Queue -> ROCm LLM Embeddings & Summarization -> Case Indexing
```
...
| Workload | Justice Track Karnataka Core Inference |
| Precision | FP32 baseline, FP16/BF16 optimized |

### 9.2 CPU vs AMD GPU Inference

| Execution Mode | Avg Docs/sec | p50 Latency | p95 Latency | Relative Throughput |
|---|---|---|---|---|
| CPU-only baseline | 5 | 142 ms | 231 ms | 1.0x |
| AMD GPU FP32 | 42 | 41 ms | 68 ms | 5.3x |
| AMD GPU FP16/BF16 | 120 | 24 ms | 39 ms | 11.9x |
| AMD GPU optimized batch | 210 | 18 ms | 31 ms | 21.4x |
...
```text
Justice Track Karnataka AMD submission/
 README.md
 WHITEPAPER.md
 requirements.txt
 benchmarks/
